...
class TicketListView(NotManagerAndLoginRequiredMixin, generic.ListView):
    ...
    def get_queryset(self):
        ...
        if user.is_organizer:
            ...
            queryset = queryset.filter(~Q(status_id__in=status_id), assigned_to__user=user, completed=False)
        else:
            project = Project.objects.filter(archive=False, organisation=user.member.organisation)
            queryset = Ticket.objects.filter(organisation=user.member.organisation, author=user, project__in=project, completed=False)
        return queryset
...
class TicketUpdateView(OrgDevAndLoginRequiredMixin, generic.UpdateView):
    ...
    def form_valid(self, form):
        ...
        if user.is_organizer:
            ...
            if (project.project_manager and ticket.project.project_manager is not None) and (project.project_manager == ticket.project.project_manager):
                if project.project_manager is not None:
                    Notification.objects.create(
                        title=f'Ticket update',
...
class TicketCompletedView(OrganizerAndLoginRequiredMixin, generic.TemplateView):
    template_name = "tickets/ticket_completed.html"

    def post(self, request, *args, **kwargs):
        user = self.request.user
        project = Project.objects.filter(archive=False, organisation=user.account)
        ticket = get_object_or_404(Ticket, project__in=project, completed=False, id=self.kwargs["pk"])
        ticket.completed = True
        ticket.save()
        ticket.project.subtract_total_score(ticket.score)
        return redirect("tickets:ticket-list")

class TicketReopenView(OrganizerAndLoginRequiredMixin, generic.TemplateView):
    template_name = "tickets/ticket_reopen.html"

    def post(self, request, *args, **kwargs):
        user = self.request.user
        project = Project.objects.filter(archive=False, organisation=user.account)
        ticket = get_object_or_404(Ticket, project__in=project, completed=True, id=self.kwargs["pk"])
...
class TicketRequestChangeView(TstrDevAndLoginRequiredMixin, generic.TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if user.role == 'tester':
            ticket = get_object_or_404(Ticket, author=user, project__in=project, pk=self.kwargs["pk"], completed=False)
        else:
            status_id = Status.objects.filter(test_status=True, organisation=user.member.organisation).values_list('id', flat=True)
            member = Member.objects.get(user_id=user.id)
            ticket = get_object_or_404(Ticket, ~Q(status_id__in=status_id), assigned_to=member, project__in=project, pk=self.kwargs["pk"], completed=False)
        if ticket.project.project_manager is not None:
            Notification.objects.create(
