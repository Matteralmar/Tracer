...
class DashboardChartView(LoginRequiredMixin, generic.ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if user.is_organizer:
            ...
            status_id = Ticket.objects.filter(~Q(status__test_status=True), assigned_to__user=user, project__in=project, status_id__isnull=False, completed=False).values('status_id')
            priority_id = Ticket.objects.filter(~Q(status__test_status=True), assigned_to__user=user, project__in=project, priority_id__isnull=False, completed=False).values('priority_id')
            type_id = Ticket.objects.filter(~Q(status__test_status=True), assigned_to__user=user, project__in=project, type_id__isnull=False, completed=False).values('type_id')
            context["statuses"] = Status.objects.filter(pk__in=status_id)
            context["count_statuses"] = status_id.annotate(total=Count('status_id'))
            context["priorities"] = Priority.objects.filter(pk__in=priority_id)
            context["count_priorities"] = priority_id.annotate(total=Count('priority_id'))
            ...
            project = Project.objects.filter(archive=False, organisation=user.member.organisation)
            status_id = Ticket.objects.filter(Q(author=user) | Q(tester=user.id, status__test_status=True), project__in=project, status_id__isnull=False, completed=False).values('status_id')
            priority_id = Ticket.objects.filter(Q(author=user) | Q(tester=user.id, status__test_status=True), project__in=project, priority_id__isnull=False, completed=False).values('priority_id')
            type_id = Ticket.objects.filter(Q(author=user) | Q(tester=user.id, status__test_status=True), project__in=project, type_id__isnull=False, completed=False).values('type_id')
            context["statuses"] = Status.objects.filter(pk__in=status_id)
            context["count_statuses"] = status_id.annotate(total=Count('status_id'))
            context["priorities"] = Priority.objects.filter(pk__in=priority_id)
...
class ProjectManagementView(ManagerAndLoginRequiredMixin, generic.ListView):
    ...
    def get_queryset(self):
        ...
        queryset = Ticket.objects.filter(Q(assigned_to__isnull=False) | Q(assigned_to__isnull=True, completed=True),project__in=project)
        return queryset

    def get_context_data(self, **kwargs):
        context = super(ProjectManagementView, self).get_context_data(**kwargs)
        id = self.request.path.split('/')[4]
        tickets = Ticket.objects.filter(project=id, assigned_to__isnull=True, completed=False)
...
class AssignMemberView(ManagerAndLoginRequiredMixin, generic.FormView):
    ...
    def get_form_kwargs(self, **kwargs):
        ...
        user = self.request.user
        ticket = get_object_or_404(Ticket, pk=self.kwargs["pk"], completed=False)
...
class ManagementTicketCreateView(ManagerAndLoginRequiredMixin, generic.CreateView):
    ...
    def form_valid(self, form):
        ...
        ticket.author = user
        ticket.save()
        ticket.project.add_total_score(ticket.score)
...
class ManagementTicketUpdateView(ManagerAndLoginRequiredMixin, generic.UpdateView):
    ...
    def get_queryset(self):
        ...
        queryset = Ticket.objects.filter(project__in=project, completed=False)
        return queryset
    ...
    def form_valid(self, form):
        ...
        if score > ticket.score:
            value = score - ticket.score
            ticket.project.add_total_score(value)
        elif score < ticket.score:
            value = ticket.score - score
            ticket.project.subtract_total_score(value)
...
class ManagementTicketCompletedView(ManagerAndLoginRequiredMixin, generic.TemplateView):
    template_name = "dashboard/management_ticket_completed.html"

    def post(self, request, *args, **kwargs):
        user = self.request.user
        project = get_object_or_404(Project, project_manager__user=user, organisation=user.member.organisation, archive=False, id=self.request.session['project_id'])
        ticket = get_object_or_404(Ticket, project=project, completed=False, id=self.kwargs["pk"])
        ticket.completed = True
        ticket.save()
        project.subtract_total_score(ticket.score)
        id = self.request.session['project_id']
        return redirect(f"/dashboard/project/management/{id}/")

class ManagementTicketReopenView(ManagerAndLoginRequiredMixin, generic.TemplateView):
    template_name = "dashboard/management_ticket_reopen.html"

    def post(self, request, *args, **kwargs):
        user = self.request.user
        project = get_object_or_404(Project, project_manager__user=user, organisation=user.member.organisation, archive=False, id=self.request.session['project_id'])
        ticket = get_object_or_404(Ticket, project=project, completed=True, id=self.kwargs["pk"])
        user = User.objects.get(username=user.member.organisation)
        Notification.objects.create(
            title=f'Request to Reopen',
            text=f'{self.request.user.username} requested to reopen a "{ticket.title}" ticket. Please contact {self.request.user.username} for more details.',
            recipient=user
        )
        id = self.request.session['project_id']
        return redirect(f"/dashboard/project/management/{id}/")
...
class ManagementTicketDeleteView(ManagerAndLoginRequiredMixin, generic.DeleteView):
    ...
    def form_valid(self, form):
        user = self.request.user
        ticket = Ticket.objects.get(pk=self.kwargs["pk"])
        ticket.project.subtract_total_score(ticket.score)
        return super(ManagementTicketDeleteView, self).form_valid(form)
...
class ProjectTestView(TesterAndLoginRequiredMixin, generic.ListView):
    ...
    def get_queryset(self):
        ...
        queryset = Ticket.objects.filter(tester=user.id, status_id__in=status_id, project__in=proj, project__archive=False, completed=False)
        return queryset
...
class TestTicketUpdateView(TesterAndLoginRequiredMixin, generic.UpdateView):
    ...
    def get_queryset(self):
        ...
        project = Project.objects.filter(archive=False, organisation=user.member.organisation)
        status_id = Status.objects.filter(test_status=True, organisation=user.member.organisation).values_list('id', flat=True)
        queryset = Ticket.objects.filter(tester=user.id, status_id__in=status_id, project__in=project, completed=False)
        return queryset
...
class TicketRequestChangeView(TesterAndLoginRequiredMixin, generic.TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        project = Project.objects.filter(archive=False, organisation=user.member.organisation)
        status_id = Status.objects.filter(test_status=True, organisation=user.member.organisation).values_list('id', flat=True)
        ticket = get_object_or_404(Ticket, tester=user.id, status_id__in=status_id, project__in=project, id=self.kwargs["pk"], completed=False)
        if ticket.project.project_manager is not None:
            Notification.objects.create(
                title=f'Request Change',
